...
export class forward extends plugin {
  ...
  async doForward() {
    ...
    groups.map(async (item) => {
      /** 转发内容 */
      Bot.pickGroup(String(item.groupId))
  ...
  async forwardForQuit(e) {
    ...
    groups.map(async (item) => {
      Bot.pickGroup(String(item.groupId)).quit();
  ...
  async forwardForPoke(that) {
    ...
    groups.map(async (item) => {
      await Bot.pickGroup(String(item.groupId)).pokeMember(uid);
      that.e.reply(`已戳[${uid}]`);
      await common.sleep(600);
      return item;
    });
